...
void MachineTests::memory() {
    ...
    QCOMPARE(m.read_byte(address), (uint8_t)0);
    QCOMPARE(m.read_hword(address), (uint16_t)0);
    QCOMPARE(m.read_word(address), (uint32_t)0);
    // Just a byte
    m.write_byte(address, 0x42);
    QCOMPARE(m.read_byte(address), (uint8_t)0x42);
    // Half word
    m.write_hword(address, 0x4243);
    QCOMPARE(m.read_hword(address), (uint16_t)0x4243);
    // Word
    m.write_word(address, 0x42434445);
    QCOMPARE(m.read_word(address), (uint32_t)0x42434445);
}
...
void MachineTests::memory_section() {
    ...
    QCOMPARE(s->read_byte(address & 0xFF), (uint8_t)0x42);
    // Write some other data trough section
    s->write_byte(address & 0xFF, 0x66);
    // Read trough memory
    QCOMPARE(m.read_byte(address), (uint8_t)0x66);
}
...
void MachineTests::memory_endian() {
    ...
    QCOMPARE(m.read_hword(0x00), (uint16_t)0x1234);
    QCOMPARE(m.read_word(0x00), (uint32_t)0x12345678);

    m.write_hword(0x80, 0x1234);
    QCOMPARE(m.read_byte(0x80), (uint8_t)0x12);
    QCOMPARE(m.read_byte(0x81), (uint8_t)0x34);

    m.write_word(0xF0, 0x12345678);
    QCOMPARE(m.read_byte(0xF0), (uint8_t)0x12);
    QCOMPARE(m.read_byte(0xF1), (uint8_t)0x34);
    QCOMPARE(m.read_byte(0xF2), (uint8_t)0x56);
    QCOMPARE(m.read_byte(0xF3), (uint8_t)0x78);
}
...
void MachineTests::memory_write_ctl_data() {
    ...
    QTest::newRow("byte") << AC_BYTE << mem;
    QTest::newRow("byte-unsigned") << AC_BYTE_UNSIGNED << mem;
    mem.write_hword(0x20, 0x2526);
    QTest::newRow("halfword") << AC_HALFWORD << mem;
    QTest::newRow("haldword-unsigned") << AC_HALFWORD_UNSIGNED << mem;
    mem.write_word(0x20, 0x23242526);
    QTest::newRow("word") << AC_WORD << mem;
}
...
void MachineTests::memory_read_ctl_data() {
    ...
    QTest::newRow("byte") << AC_BYTE << (uint32_t)0xFFFFFFA3;
    QTest::newRow("halfword") << AC_HALFWORD << (uint32_t)0xFFFFA324;
    QTest::newRow("word") << AC_WORD << (uint32_t)0xA3242526;
    QTest::newRow("byte-unsigned") << AC_BYTE_UNSIGNED << (uint32_t)0xA3;
    QTest::newRow("halfword-unsigned")
        << AC_HALFWORD_UNSIGNED << (uint32_t)0xA324;
}
