...
TESTS = [
    ...
    },
    {
        ...
        "min_cycles": 30,
        "max_cycles": 300,
        "skip_if_missing": True,  # May be skipped for Clang builds
    },
    {
        "name": "linus_hello",
...
def get_register_value(registers, reg_name):
    ...
    if r_name in registers:
        return registers[r_name]
...
def verify_result(test, result, verbose):
    ...
    registers = {
        ...
        for m in re.finditer(r"([A-Za-z0-9_]+):0x([0-9a-fA-F]+)", result.stdout)
    }
    ...
    for reg, expected in test.get("expected_registers", {}).items():
        ...
        if actual is None:
            return False, f"Register {reg} not found" + (
                f"\nAvailable: {list(registers.keys())}" if verbose else ""
            )
        if actual != expected:
            return (
                False,
                f"Register {reg}: expected 0x{expected:x}, got 0x{actual:x}"
    ...
    if test.get("marker") and test["marker"] not in result.stdout:
        return False, f"Marker '{test['marker']}' not found" + (
            f"\nStdout: {result.stdout}" if verbose else ""
        )
...
def run_test(qtrvsim_cli, test, elf_dir, verbose=False, pipeline=False, cache=False):
    """Run a single test. Returns (success, message, skipped) tuple."""
    elf_path = os.path.join(elf_dir, os.path.basename(test["elf_path"]))
    if not os.path.exists(elf_path):
        if test.get("skip_if_missing"):
            return True, "SKIPPED (ELF not built)", True
        return False, f"ELF not found: {elf_path}", False

    cmd = [qtrvsim_cli, elf_path, "--dump-registers", "--dump-cycles"]
    if pipeline:
        cmd.append("--pipelined")
    if cache:
        cmd.extend(["--d-cache", "lru,2,2,2", "--i-cache", "lru,2,2,2"])

    try:
        result = subprocess.run(
            cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=30
        )
        success, msg = verify_result(test, result, verbose)
        return success, msg, False
    except subprocess.TimeoutExpired:
        return False, "Test timed out (30s)", False
...
def main():
    ...
    parser.add_argument(
        "--build-dir", required=True, help="Directory containing test ELFs"
    )
    parser.add_argument(
        "--qtrvsim-cli", required=True, help="Path to qtrvsim_cli executable"
    )
    parser.add_argument(
        "--stud-support-path", required=True, help="stud-support repo path"
    )
    ...
    mode = (
        "pipelined+cached"
        if args.pipeline and args.cache
        else "pipelined"
        if args.pipeline
        else "cached"
        if args.cache
    ...
    print(
        f"Running {len(tests)} tests ({mode})\nUsing: {qtrvsim_cli}\nELFs: {elf_dir}\n"
        + "=" * 70
    )

    results = [
        run_test(qtrvsim_cli, t, elf_dir, args.verbose, args.pipeline, args.cache)
        for t in tests
    ]
    passed = sum(1 for r in results if r[0] and not r[2])
    skipped = sum(1 for r in results if r[2])
    failed = sum(1 for r in results if not r[0])

    for i, (success, msg, was_skipped) in enumerate(results):
        if was_skipped:
            status = "[SKIP]"
        elif success:
            status = "[OK]  "
    ...
    print("=" * 70 + f"\nResults: {passed}/{len(tests) - skipped} passed", end="")
    if skipped:
    ...
    if failed > 0:
        print("\nFailed tests:")
        for i, (success, _, was_skipped) in enumerate(results):
            if not success and not was_skipped:
                print(f"  - {tests[i]['name']}")
