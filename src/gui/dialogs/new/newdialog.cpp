...
void NewDialog::bp_type_change() {
    ...
    case machine::PredictorType::SMITH_1_BIT: {
        ...
        const int index{
            ui->select_bp_init_state->findData(QVariant::fromValue(config->get_bp_init_state()))
        };
        if (index >= 0) {
            ...
            ui->select_bp_init_state->setCurrentIndex(
                ui->select_bp_init_state->findData(QVariant::fromValue(machine::PredictorState::NOT_TAKEN))
            );
    ...
    case machine::PredictorType::SMITH_2_BIT_HYSTERESIS: {
        ...
        if (index >= 0) {
            ...
            ui->select_bp_init_state->setCurrentIndex(
                ui->select_bp_init_state->findData(QVariant::fromValue(machine::PredictorState::WEAKLY_NOT_TAKEN))
            );
    ...
    default:
        bp_toggle_history_table_ui(false);
        break;
    }
}
...
void NewDialog::config_gui() {
    ...
    const int index{
        ui->select_bp_type->findData(QVariant::fromValue(config->get_bp_type()))
    };
    if (index >= 0) {
