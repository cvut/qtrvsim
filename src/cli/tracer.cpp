...
void trace_instruction_in_stage(
    ...
    printf(
        ...
        qPrintable(stage.inst.to_str(stage.inst_addr)), qPrintable(source_info));
}
...
void Tracer::step_output() {
    ...
    if (trace_fetch) {
        trace_instruction_in_stage("Fetch", if_id, debug_info, &debug_hint, trace_source);
    }
    if (trace_decode) {
        trace_instruction_in_stage("Decode", id_ex, debug_info, &debug_hint, trace_source);
    }
    if (trace_execute) {
        trace_instruction_in_stage("Execute", ex_mem, debug_info, &debug_hint, trace_source);
    }
    if (trace_memory) {
        trace_instruction_in_stage("Memory", mem_wb, debug_info, &debug_hint, trace_source);
    }
    if (trace_writeback) {
        // All exceptions are resolved in memory, therefore there is no excause field in WB.
        QString source_info;
        if (trace_source && debug_info && wb.inst_addr != STAGEADDR_NONE) {
