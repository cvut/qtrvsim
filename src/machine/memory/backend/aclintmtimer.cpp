...
uint64_t AclintMtimer::mtime_fetch_current() const {
    ...
    mtime_last_current_fetch = mtime_start_offset.msecsTo(current_time) *
                      (uint64_t)10000;
...
void AclintMtimer::arm_mtimer_event() {
    ...
    uint64_t ticks_to_wait = mtimecmp_value[0] -
            (mtime_last_current_fetch + mtime_user_offset);
    qt_timer_id =startTimer(ticks_to_wait / 10000);
}

WriteResult AclintMtimer::write(
    Offset destination,
    const void *source,
    size_t size,
...
bool AclintMtimer::write_reg64(Offset destination, uint64_t value) {
    ...
    if (destination == ACLINT_MTIME_OFFSET) {
        ...
        if (!update_mtimer_irq())
            arm_mtimer_event();
        changed = true;
    } else if ((destination >= ACLINT_MTIMECMP_OFFSET) &&
