...
// This header documents the SV32 layout (RISC-V 32-bit virtual memory):
...
//    page number (PPN).
namespace machine {
    static constexpr unsigned PAGE_SHIFT = 12; // Page size = 2^PAGE_SHIFT bytes. For SV32 this is 4 KiB.
    static constexpr unsigned VPN_BITS = 10;  // Number of bits for each VPN level in SV32: 10 bits for VPN[1] and 10 bits for VPN[0].

    // Shift values for extracting VPN parts from a virtual address:
    static constexpr unsigned VPN0_SHIFT = PAGE_SHIFT; // VPN0 is the low VPN field, it starts at the page offset (PAGE_SHIFT).
    static constexpr unsigned VPN1_SHIFT = PAGE_SHIFT + VPN_BITS; // VPN1 is the next field above VPN0.

    static constexpr unsigned VPN_MASK = (1u << VPN_BITS) - 1;  // Mask to extract a single VPN level (10 bits, value 0..1023).

    // Number of bits available for the physical page number (PPN) in SV32 PTE.
    // SV32 uses 22 PPN bits (bits 10..31 of a 32-bit PTE) which permits addressing up to 4GiB of physical memory.
    ...
    struct Sv32Pte {
        ...
        static constexpr Sv32Pte make(bool v_, bool r_, bool w_, bool x_, bool u_, bool g_, bool a_, bool d_, uint64_t rsw_, uint64_t ppn_) {
            uint64_t r = 0;
