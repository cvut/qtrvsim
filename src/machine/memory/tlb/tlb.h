...
public:
    ...
    uint32_t root_page_table_ppn() const {
        return current_satp_raw & ((1u << PPN_BITS) - 1);
    }
...
signals:
    void tlb_update(unsigned way, unsigned set, bool valid, uint16_t asid, uint64_t vpn, uint64_t phys_base, bool is_write);
