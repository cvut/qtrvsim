...
// Find instruction address in the BTB using the provided instruction address, return 0 address if not found
...
// Find target address in the BTB using the provided instruction address, return 0 address if not found
...
PredictorSmith::PredictorSmith(uint8_t number_of_bht_addr_bits, uint8_t number_of_bht_bits, PredictorState initial_state) : 
...
uint8_t PredictorSmith::init_number_of_bht_addr_bits(const uint8_t b) const {
    if (b > BP_MAX_BHT_ADDR_BITS) {
        WARN("Number of BHT bits from incstruction address (%u) was larger than %d during init", b, BP_MAX_BHT_ADDR_BITS);
...
uint16_t PredictorSmith::calculate_bht_index(const uint16_t bhr_value, const Address instruction_address) const {
...
PredictorSmith1Bit::PredictorSmith1Bit(uint8_t number_of_bht_addr_bits, uint8_t number_of_bht_bits, PredictorState initial_state) : PredictorSmith(number_of_bht_addr_bits, number_of_bht_bits, initial_state) {};
...
PredictorSmith2Bit::PredictorSmith2Bit(uint8_t number_of_bht_addr_bits, uint8_t number_of_bht_bits, PredictorState initial_state) : PredictorSmith(number_of_bht_addr_bits, number_of_bht_bits, initial_state) {};
...
PredictorSmith2BitHysteresis::PredictorSmith2BitHysteresis(uint8_t number_of_bht_addr_bits, uint8_t number_of_bht_bits, PredictorState initial_state) : PredictorSmith(number_of_bht_addr_bits, number_of_bht_bits, initial_state) {};
    ...
    number_of_bht_bits(init_number_of_bht_bits(number_of_bhr_bits, number_of_bht_addr_bits))
{
    ...
    case PredictorType::SMITH_1_BIT:
        ...
        LOG("Initialized branch predictor: %s, with %u BHT bits, and initial state at: %s", predictor->get_name(), number_of_bht_bits, predictor_state_to_string(initial_state));
        break;
    ...
    if (enabled) {
        connect(btb, &BranchTargetBuffer::requested_target_address, this, &BranchPredictor::requested_bht_target_address);
        connect(btb, &BranchTargetBuffer::update_row_done, this, &BranchPredictor::update_btb_row_done);
        ...
        connect(predictor, &Predictor::update_stats_done, this, &BranchPredictor::update_predictor_stats_done);
        connect(predictor, &Predictor::update_bht_row_done, this, &BranchPredictor::update_predictor_bht_row_done);
    }
}
...
uint8_t BranchPredictor::init_number_of_bht_addr_bits(const uint8_t b) const {
    if (b > BP_MAX_BHT_ADDR_BITS) {
        WARN("Number of BHT instruction address bits (%u) was larger than %d during init", b, BP_MAX_BHT_ADDR_BITS);
        return BP_MAX_BHT_ADDR_BITS;
    }
    return b;
}
...
Address BranchPredictor::predict_next_pc_address(const Instruction instruction, const Address instruction_address) const {
    ...
    if (instruction_address_from_btb.is_null() || instruction_address != instruction_address_from_btb) {
        return instruction_address + 4;
    }

    // Get target address from BTB
    Address target_address_from_btb = btb->get_target_address(instruction_address);
    if (target_address_from_btb.is_null()) {
        return instruction_address + 4;
    }
    ...
    if (predicted_result == BranchResult::TAKEN) {
        return target_address_from_btb;
    }
...
void BranchPredictor::update(const Instruction instruction, const Address instruction_address, const Address target_address, const BranchResult result) {
    // Check if predictor is enabled
    if (!enabled) {
        return;
    }
    ...
    if (result == BranchResult::TAKEN) {
        btb->update(instruction_address, target_address);
    }
