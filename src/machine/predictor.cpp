...
uint8_t BranchHistoryRegister::init_number_of_bits(const uint8_t number_of_bits) const {
    if (number_of_bits > PREDICTOR_MAX_TABLE_BITS) {
        WARN("Number of BHR bits (%u) was larger than %u during init", number_of_bits, PREDICTOR_MAX_TABLE_BITS);
...
uint8_t BranchTargetTable::init_number_of_bits(const uint8_t number_of_bits) const {
    if (number_of_bits > PREDICTOR_MAX_TABLE_BITS) {
        WARN("Number of BTT bits (%u) was larger than %u during init", number_of_bits, PREDICTOR_MAX_TABLE_BITS);
        return PREDICTOR_MAX_TABLE_BITS;
    }
    return number_of_bits;
}
...
void BranchTargetTable::print() const {
    ...
    for(const BranchTargetTableEntry& entry : data) {
        LOG("INSTR: %3lu, TARGET: %3lu", entry.instruction_address.get_raw(), entry.target_address.get_raw());
    }
    LOG("---------------");
}
...
void BranchTargetTable::update(const uint16_t index, const Address instruction_address, const Address target_address) {
    ...
    if (is_address_in_table(instruction_address)) {
        return;
    }

    // Prepare new BTT entry
    const BranchTargetTableEntry new_entry{
...
void Predictor::update_stats(PredictionFeedback feedback) {
    ...
    if ((stats.number_of_correct_predictions + stats.number_of_wrong_predictions) > 0) {
        stats.accuracy = (100 * stats.number_of_correct_predictions) / (stats.number_of_correct_predictions + stats.number_of_wrong_predictions);
    }
    else {
        stats.accuracy = 0;
    }
...
BranchResult PredictorBTFNT::make_prediction(PredictionInput input) const {
    if (input.target_address > input.instruction_address) {
        // If target address is larger than jump instruction address (forward jump), predict not taken
        return BranchResult::NOT_TAKEN;
    }
    else {
...
void PredictorBTFNT::update(PredictionFeedback feedback) {
    if (stats.last_prediction == BranchResult::UNDEFINED) {
        stats.last_prediction = make_prediction({
...
PredictorSmith::PredictorSmith(uint8_t number_of_bht_bits, PredictorState initial_state) : number_of_bht_bits(init_number_of_bht_bits(number_of_bht_bits)) {
...
uint8_t PredictorSmith::init_number_of_bht_bits(uint8_t number_of_bht_bits) const {
    if (number_of_bht_bits > PREDICTOR_MAX_TABLE_BITS) {
        WARN("Number of BHT bits (%u) was larger than %d during init", number_of_bht_bits, PREDICTOR_MAX_TABLE_BITS);
        return PREDICTOR_MAX_TABLE_BITS;
    }
...
void PredictorSmith::update_stats(PredictionFeedback feedback) {
    ...
    if ((row_stats.number_of_correct_predictions + row_stats.number_of_wrong_predictions) > 0) {
        row_stats.accuracy = (100 * row_stats.number_of_correct_predictions) / (row_stats.number_of_correct_predictions + row_stats.number_of_wrong_predictions);
        stats.accuracy = (100 * stats.number_of_correct_predictions) / (stats.number_of_correct_predictions + stats.number_of_wrong_predictions);
    }
    else {
...
PredictorSmith1Bit::PredictorSmith1Bit(uint8_t number_of_bht_bits, PredictorState initial_state) : PredictorSmith(number_of_bht_bits, initial_state) {};
...
PredictorSmith2Bit::PredictorSmith2Bit(uint8_t number_of_bht_bits, PredictorState initial_state) : PredictorSmith(number_of_bht_bits, initial_state) {};
...
PredictorSmith2BitHysteresis::PredictorSmith2BitHysteresis(uint8_t number_of_bht_bits, PredictorState initial_state) : PredictorSmith(number_of_bht_bits, initial_state) {};
...
void PredictorSmith2BitHysteresis::update_bht(PredictionFeedback feedback) {
    ...
    if(stats.last_prediction == BranchResult::UNDEFINED || bht.at(feedback.bht_index).stats.last_prediction == BranchResult::UNDEFINED) {
...
BranchPredictor::BranchPredictor(bool enabled, PredictorType predictor_type, PredictorState initial_state, uint8_t number_of_bhr_bits, uint8_t number_of_address_bits) : 
    enabled(enabled),
    ...
    if (number_of_address_bits > PREDICTOR_MAX_TABLE_BITS) {
        throw std::invalid_argument("Branch predictor initialization: Invalid number of address index bits");
    }

    if (number_of_table_bits > PREDICTOR_MAX_TABLE_BITS) {
        throw std::invalid_argument("Branch predictor initialization: Invalid sum of number of BHR and address bits");
    }
    ...
    case PredictorType::SMITH_1_BIT:
        ...
        LOG("Initialized branch predictor: %s, with %u BHT bits, and initial state at: %s", predictor->get_name(), number_of_table_bits, predictor_state_to_string(initial_state));
        break;
    ...
    default:
        throw std::invalid_argument("Invalid predictor type selected");
    }
    ...
    if (enabled) {
        ...
        connect(predictor, &Predictor::updated_stats, this, &BranchPredictor::updated_predictor_stats);
...
uint8_t BranchPredictor::init_number_of_bhr_bits(const uint8_t number_of_bhr_bits) const {
    if (number_of_bhr_bits > PREDICTOR_MAX_TABLE_BITS) {
        WARN("Number of BHR bits (%u) was larger than %d during init", number_of_bhr_bits, PREDICTOR_MAX_TABLE_BITS);
        return PREDICTOR_MAX_TABLE_BITS;
    }
    return number_of_bhr_bits;
}

uint8_t BranchPredictor::init_number_of_address_bits(const uint8_t number_of_bhr_bits, const uint8_t number_of_address_bits) const {
    ...
    if (number_of_address_bits > PREDICTOR_MAX_TABLE_BITS) {
        WARN("Number of address index bits (%u) was larger than %d during init", number_of_address_bits, PREDICTOR_MAX_TABLE_BITS);
    ...
    if ((checked_number_of_bhr_bits + checked_number_of_address_bits) > PREDICTOR_MAX_TABLE_BITS) {
        WARN("Sum of BHR and address index bits (%u) was larger than %d during init", number_of_address_bits, PREDICTOR_MAX_TABLE_BITS);
...
uint8_t BranchPredictor::init_number_of_table_bits(const uint8_t number_of_bhr_bits, const uint8_t number_of_address_bits) const {
    // Clamp number of BHR bits
    uint8_t checked_number_of_bhr_bits = number_of_bhr_bits;
    if (number_of_bhr_bits > PREDICTOR_MAX_TABLE_BITS) {
        checked_number_of_bhr_bits = PREDICTOR_MAX_TABLE_BITS;
    ...
    // Check sum
    if ((checked_number_of_bhr_bits + checked_number_of_address_bits) > PREDICTOR_MAX_TABLE_BITS) {
        WARN("Number of BTT and BHT index bits (%u) was larger than %d during init", number_of_address_bits, PREDICTOR_MAX_TABLE_BITS);
        return PREDICTOR_MAX_TABLE_BITS;
    }
...
PredictorType BranchPredictor::get_predictor_type() const {
    if (!enabled) {
        return PredictorType::UNDEFINED;
    }
...
PredictorState BranchPredictor::get_initial_state() const {
    if (!enabled) {
        return PredictorState::UNDEFINED;
    }
...
Address BranchPredictor::predict_next_pc_address(const Instruction instruction, const Address instruction_address) const {
...
void BranchPredictor::update_result(const Instruction instruction, const Address instruction_address, const BranchResult result) {
    // Check if predictor is enabled
    if (!enabled) {
        return;
    }
