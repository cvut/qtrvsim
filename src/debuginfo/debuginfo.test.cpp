...
void TestDebugInfo::test_sequence_gaps() {
    DebugInfo info;
    auto file = info.get_file_id("test.c");
    info.add_line(0x100, file, 10);
    info.add_end_sequence(0x108);
    info.add_line(0x200, file, 20);
    info.add_end_sequence(0x204);
    info.finalize();

    for (size_t initial_hint : { size_t(0), size_t(1), size_t(3), size_t(9999) }) {
        size_t hint = initial_hint;
        for (uint64_t address : { 0x107, 0x108, 0x180, 0x200, 0x204, 0x100, 0x208 }) {
            const auto *loc = info.find(address, &hint);
            const auto *unhinted = info.find(address);
            QCOMPARE(loc, unhinted);
            if (address == 0x107 || address == 0x100 || address == 0x200) {
                QVERIFY(loc != nullptr);
            } else {
                QVERIFY(loc == nullptr);
            }
...
void TestDebugInfo::test_shared_sequence_boundary() {
    for (bool end_first : { false, true }) {
        DebugInfo info;
        auto first = info.get_file_id("first.c");
        auto second = info.get_file_id("second.c");
        info.add_line(0x100, first, 10);
        if (end_first) { info.add_end_sequence(0x104); }
        info.add_line(0x104, second, 20);
        if (!end_first) { info.add_end_sequence(0x104); }
        info.add_end_sequence(0x108);
        info.finalize();
        info.finalize();
        const auto *loc = info.find(0x104);
        QVERIFY(loc != nullptr);
        QCOMPARE(loc->file_id, second);
        QCOMPARE(loc->line, 20u);
        QVERIFY(info.find(0x108) == nullptr);
    }
}

void TestDebugInfo::test_duplicate_line_rows() {
    DebugInfo info;
    auto file = info.get_file_id("test.c");
    info.add_line(0x100, file, 10);
    info.add_line(0x100, file, 20);
    info.add_end_sequence(0x104);
    info.finalize();
    const auto *loc = info.find(0x100);
    QVERIFY(loc != nullptr);
    QCOMPARE(loc->line, 20u);
}

void TestDebugInfo::test_unattributed_line() {
    DebugInfo info;
    auto file = info.get_file_id("test.c");
    info.add_line(0x100, file, 10);
    info.add_line(0x104, file, 0);
    info.add_line(0x108, file, 20);
    info.add_end_sequence(0x10c);
    info.finalize();
    size_t hint = 0;
    QVERIFY(info.find(0x104, &hint) == nullptr);
    QVERIFY(info.find(0x107) == nullptr);
    QVERIFY(info.find(0x108, &hint) != nullptr);
}
