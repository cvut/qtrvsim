...
namespace {
    constexpr FileId NO_FILE = std::numeric_limits<FileId>::max();
}
...
void DebugInfo::add_end_sequence(uint64_t address) {
    add_line(address, NO_FILE, 0);
}

const SourceLocation *DebugInfo::find(uint64_t address, size_t *hint_index) const {
    if (addresses.empty()) { return nullptr; }

    auto location_at = [this](size_t index) -> const SourceLocation * {
        return locations[index].line == 0 ? nullptr : &locations[index];
    };
...
void DebugInfo::finalize() {
    ...
    std::sort(permutation.begin(), permutation.end(), [this](size_t a, size_t b) {
        ...
        const bool a_end = locations[a].file_id == NO_FILE;
        const bool b_end = locations[b].file_id == NO_FILE;
